...
public class Manager {
    ...
    private List<User> AplicattionUsers;
    ...
    public User getUser(String userName) {
        for (User user : this.AplicattionUsers) {
    ...
    public void addUser(User user) {
        AplicattionUsers.add(user);
    }

    public void deleteUser(User user) {
        AplicattionUsers.remove(user);
    }

    public void init() {
        //Creates a list of users and add them to the application
        this.AplicattionUsers = this.createUsers();
    ...
    public void createPosts() {
        ...
        for (User user : this.AplicattionUsers) {
            //añadiria el objeto post1 a la lista de post del usuario, pero para la siguiente iteracion meteria el mismo objeto en la
            //lista de post de otro usuario, por lo que se deberia crear un nuevo objeto post para cada usuario (no pasaria si hay base de datos) 
    ...
    public List<Section> createSections() {
        List<Section> sections = new ArrayList<>(Arrays.asList(
                new Section("Reversing", "Análisis y descompilación de binarios para entender su funcionamiento.", "reversing.jpg"),
                new Section("Hacking Web", "Explotación de vulnerabilidades en aplicaciones web.", "hacking_web.jpg"),
                new Section("Escalada de Privilegios en Linux", "Técnicas para obtener mayores privilegios en sistemas Linux.", "escalada_linux.jpg"),
                new Section("Escalada de Privilegios en Windows", "Métodos para obtener acceso administrativo en Windows.", "escalada_windows.jpg"),
